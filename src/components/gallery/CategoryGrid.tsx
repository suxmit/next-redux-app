          ...
          {products.map((category) => {
             ...
             return (
                <Link key={category.id} href={`/products/${category.slug}`} className="group block">
                    <div className="relative aspect-[4/3] overflow-hidden rounded-sm bg-gray-100 mb-2 md:mb-4">
                        {firstProductImage ? (
                            <Image 
                    ...
                    <h4 className="text-base md:text-xl font-bold text-forest group-hover:text-gold transition-colors mb-0.5 md:mb-1">
                        {category.name}
                    </h4>
                    <span className="text-[10px] sm:text-xs md:text-sm text-gray-500 group-hover:text-gray-700 transition-colors">
